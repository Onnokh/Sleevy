...
final class ReadingListStore: ObservableObject {
    ...
    private let pathMonitorQueue = DispatchQueue(label: "plowplow.Sleeve.ReadingListStore.pathMonitor")
    ...
    private func submitReadStateUpdate(itemId: String, isRead: Bool) async throws -> SavedItem {
        ...
        guard (200 ..< 300).contains(httpResponse.statusCode) else {
            let message = serverMessage(data) ?? "Sleeve could not update this saved item right now."
