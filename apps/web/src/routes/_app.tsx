  ...
  return (
    <div className="sign-in">
      ...
      <div className="sign-in-content">
        <img src={brandmarkWhiteUrl} alt="Sleevy" className="sign-in-logo" width={54} height={80} />
        <button type="button" className="sign-in-button sign-in-button--primary" disabled={isSigningIn} onClick={() => void startSignIn("apple")}>
          <AppleIcon />
          {signingInProvider === "apple" ? "Opening Apple..." : "Continue with Apple"}
        </button>
        <button type="button" className="sign-in-button sign-in-button--secondary" disabled={isSigningIn} onClick={() => void startSignIn("google")}>
          {signingInProvider === "google" ? "Opening Google..." : "Continue with Google"}
        </button>
        {error ? <pre className="sign-in-error">{error}</pre> : null}
      </div>
    </div>
  )
}
...
function SignInBokeh() {
  ...
  return <canvas ref={canvasRef} className="sign-in-bokeh" aria-hidden="true" />
}
